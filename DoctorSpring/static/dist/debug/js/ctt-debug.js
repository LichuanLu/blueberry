...
define('admin/show/show_view',["backbone","marionette"],function (Backbone,Marionette) {
	// body...
	...
	var adminView = Marionette.ItemView.extend({
		...
		events:{
			"click @ui.button":"sayHandler"  //-- this should work on latest version of marionette
			//"click #say":"sayHandler"
		},
		sayHandler:function () {
			// body...
			alert(this.ui.input.val());
...
define('admin/show/show_controller',['config/controllers/_base_controller','admin/show/show_view'],function (BaseController,View) {
	// body...
	
	var ShowController = BaseController.extend({
		initialize:function () {
			// body...
			var adminview = this.getAdminView();
			this.show(adminview);
			console.log('show controller init');

		},
		getAdminView:function() {
			// body...
			return new View.adminView()
		}
	});
...
define('admin/admin_app',['admin/show/show_controller'],function (ShowController) {
	// body...
...
(function(){dust.register("localeItem",body_0);function body_0(chk,ctx){return chk.write("<option value=\"").reference(ctx._get(false, ["iso"]),ctx,"h").write("\">").reference(ctx._get(false, ["name"]),ctx,"h").write("--").reference(ctx._get(false, ["iso"]),ctx,"h").write("</option>");}return body_0;})();
define("project_setup/templates/localeItem", function(){});

(function(){dust.register("locales",body_0);function body_0(chk,ctx){return chk.write("<div class=\"btn-group\"><select id=\"localeList\" multiple=\"multiple\" name=\"localeList\"></select><button id=\"toggleBtn\" class=\"btn btn-primary\">Select All</button></div>");}return body_0;})();
define("project_setup/templates/locales", function(){});

define('templates',['require','project_setup/templates/localeItem','project_setup/templates/locales'],function(require) {
...
define('project_setup/show/show_view',['lodash','marionette', 'templates','dust', 'dustMarionette', "bootstrap", "bootstrap.multiselect", 'jquery.validate'], function(Lodash,Marionette, Templates) {
	// body...
	
	var setupView = Marionette.Layout.extend({
		initialize: function() {
			// body...
			this.bindUIElements();
		},
		...
		ui: {
			...
			"inProductVersion": "#inProductVersion",
			"inCodeName": "#inCodeName",
			"inLocales": "#localeList",
			"inDisName": "#inDisName",
			"inOwer": "#inOwer",
			"submitBtn": ".submit-button",
			"inRestrict":"#inRestrict"
		},
		events: {
			"click @ui.submitBtn": "submitForm" //-- this should work on latest version of marionette
			//"click #say":"sayHandler"
		},
		submitForm: function(e) {
			...
			// 	console.log(data);
				$.ajax({
					url: '/product/manage/setup/product',
					...
					onError: function() {
						this.resetForm(true);
						alert('ERROR. ERROR. Import could not be started.');
						//allowSubmit = true;
					}
				});
		},
		...
		validate: function() {
			...
			data.productOwners = this.ui.inOwer.val();
			data.iconURL = $("#downloadFile a:first").attr('href');
			data.targetIsoCodes = "";
			var localeOptions = $('#localeList > option:selected');
			localeOptions.each(function (index) {
				// body...
				if(data.targetIsoCodes == ""){
					data.targetIsoCodes = this.value;

				}else{
					data.targetIsoCodes = data.targetIsoCodes+","+this.value; 
				}
			})			
			return data
		...
		resetForm: function(leaveInputData) {
			...
			if (!leaveInputData) {
				// $inProduct.val(0);
				this.ui.inProductName.val('');
				this.ui.inCodeName.val('');
				this.ui.inProductVersion.val('');
				this.ui.inDisName.val('');
				this.ui.inOwer.val('');
				$("#inRestrict")
			}
			// $bttSubmit.css({
			//     cursor: '',
			//     opacity: 1
			// }).find('.primary').text('Do it');
		}
	});
	var localesView = Marionette.CompositeView.extend({
		template: "locales",
		//itemView: localeItemView,
		itemViewContainer: "#localeList",
		events: {
			"click #toggleBtn": "selectAll"
		},
		onRender: function() {
			// body...
			console.log("locale render");
			//$("#localeList").multiselect();
		},
		onShow: function() {
			// body...
			//have to add this in onShow not onRender , as it's inside a region , only onShow means the dom is add the browser
			$("#localeList").multiselect();
			//add the validator for the input and select
			$.validator.addMethod(
			...
			console.log("locale show");
		},
		selectAll: function(e) {
			// body...
			e.preventDefault();
			this.multiselect_toggle($("#localeList"), $(e.target));
	...
	var localeItemView = Marionette.ItemView.extend({
		template: "localeItem",
		onRender: function() {
			// body...
	...
	return {
		setupView: setupView,
		localesView: localesView,
		localeItemView: localeItemView
	}
});
define('entities/localeEntity',["backbone","marionette","config/base/constant","utils/reqcmd"],function (Backbone,Marionette,Constant,Reqcmd) {
	// body...
	...
	var API = {
		getLocalesTest: function () {
			// body...
			var res = new LocaleCollection([
			{
				name:"French",
				iso: "fr_FR"
		...
		getLocales: function (params) {
			...
			localesListData.url = Constant+"/locale";
...
define('project_setup/show/show_controller',['lodash','config/base/constant','config/controllers/_base_controller', 'project_setup/show/show_view', 'utils/reqcmd', 'entities/localeEntity','jquery.uploader.main'], function(Lodash,CONSTANT,BaseController, View, ReqCmd) {
	// body...
	...
	var ShowController = BaseController.extend({
		initialize: function() {
			...
			// body...
			Lodash.bindAll(this);
			this.localeListData = ReqCmd.reqres.request("locale:entities");

			this.layoutView = this.getSetupView();
			//as bindAll this,so don't need that
			//var that = this;
			ReqCmd.commands.setHandler("setupView:attached", function(instance) {
				// body...
				...
				$('#fileupload').fileupload({
					...
					url: 'http://localhost:8102/product/logo/upload/'
					//url: 'http://localhost:8888/php/'

				});
				instance.localesView(instance.localeListData);
				//for testing reset
				//instance.localeListData.reset();
			});
			// this.listenTo(this.layoutView,"show",function (that) {
			// 	// body...
			// 	that.localesView(that.localeListData);
			// })
		...
		localesView: function(localeListData) {
			// body...
			var Itemview = this.getLocaleItemView();
			var view = this.getLocalesView(localeListData, Itemview);
			this.show(view, {
				region: this.layoutView.localeRegion,
				client: true
			});
		},
		getSetupView: function() {
			// body...
			return new View.setupView()
		},
		getLocalesView: function(localeListData, itemview) {
			// body...
			return new View.localesView({
				collection: localeListData,
				itemView: itemview
			})
		},
		getLocaleItemView: function() {
			// body...
			return View.localeItemView
		}
...
define('project_setup/project_setup_app',['project_setup/show/show_controller'],function (ShowController) {
	// body...
...
define('login/show/show_view',['lodash', 'marionette', 'utils/reqcmd',"bootstrap",'jquery.validate'], function(Lodash, Marionette,ReqCmd) {
	// body...
	...
	var loginView = Marionette.ItemView.extend({
		initialize: function() {
			// body...
			this.bindUIElements();
			this.$("#loginForm").on("submit", this.submitForm);
			ReqCmd.commands.setHandler("loginView:attached", function(instance) {
				// body...
				console.log("login view attached end");
			   $('#loginForm').validate({
				rules: {
					j_username: {
						required: true,
		...
		submitForm: function(e) {
			...
			var data = {
				username: $("#loginForm #uInput").val(),
				password: $("#loginForm #uPassword").val()
			};
			 var loginUrl = '/rest/loginService?return_uri=' + encodeURIComponent("http://localhost:8102/static/index-sproduct.html");
			if ($("#loginForm input[name=useRememberMe]").attr("checked") == "checked") {
				loginUrl += "&useRememberMe=true";
			}
			$.ajax({
				type: 'POST',
				dataType: "json",
				...
				error: function(data) {
					if (data.status == 401) {
						...
						if (error.imsJump) {
							window.location.replace(error.imsJump);
							return;
						}
					}
					...
					try {
						...
						errorMessage = (error != null) ? ("%ERROR_" + error.errorCode) : "%ERROR_19999";
					} catch (err) {
						errorMessage = "%ERROR_19999";
					}
					$(".error-container").text(errorMessage);
					$(".error-container").show();
				}
		...
		fbLogin: function(e) {
			var $fbLogin = $("#fbLoginBtn");
			var data = $fbLogin.attr("href").split("?ims_uri=");
			var imsData = "?return_uri=" + encodeURIComponent(window.top.location.href) + "&ims_uri=" + encodeURIComponent(data[1]);
			var url = data[0] + imsData;
			$fbLogin.attr("target", "_top");
			$fbLogin.attr("href", url);
		},
		resetForm: function(leaveInputData) {
			// body...
			// if (leaveInputData !== true) {
...
define('login/show/show_controller',['lodash','config/base/constant','config/controllers/_base_controller', 'login/show/show_view', 'utils/reqcmd', 'entities/localeEntity','jquery.uploader.main'], function(Lodash,CONSTANT,BaseController, View, ReqCmd){
	// body...
	...
	var ShowController = BaseController.extend({
		initialize: function() {
			// body...
			Lodash.bindAll(this);
			this.loginView = this.getLoginView();
...
define('login/login_app',['login/show/show_controller'],function (ShowController) {
	// body...
...
//application root 
define('app',["backbone","marionette","admin/admin_app","project_setup/project_setup_app","utils/reqcmd","login/login_app"],function(Backbone,Marionette,AdminApp,ProjectSetupApp,ReqCmd,LoginApp) {
		// body...
		...
		App.addInitializer(function () {
			...
			if(location.indexOf("index-sproduct")!= -1){
				ProjectSetupApp.API.show();
			}
			else{
				LoginApp.API.show();
			}
...
require([],function () {
    ...
    beforeSend:function (xhr, settings) {
        ...
        if (settings.url.indexOf("/rest/loginService") != -1 || settings.url.indexOf("/api/") !=-1) {
    ...
    error:function (data) {
        ...
        try {
            if (this.url.indexOf("/file/dld/") == -1 && this.url.indexOf("/modules/") == -1) {
                ...
                //     text:(error != null) ? localization.getString("%ERROR_" + error.code) : localization.getString("%ERROR_19999"),
                //     sticky:false,
...
require(['config/_base','app','ajax.setup'],function(config,App){
